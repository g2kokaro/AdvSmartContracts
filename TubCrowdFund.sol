...
contract CrowdFund is Storage {
    ...
    function pledge(uint _id) public payable {
        require(now < crowdFunds[_id].deadline, "Contributions can only be made before the deadline");
        if (crowdFunds[_id].contributions[tx.origin] == 0){
            crowdFunds[_id].contributors++;
        }
        crowdFunds[_id].contributions[tx.origin] += msg.value;
        crowdFunds[_id].weiRaised += msg.value;
        emit NewContribution(msg.sender, msg.value, now);
    }

    function refund(uint _id) public {
        require(crowdFunds[_id].weiRaised < crowdFunds[_id].goalInWei && now > crowdFunds[_id].deadline,
    ...
    function getContributors(uint _id) public view returns (uint) {
        return crowdFunds[_id].contributors;
    }
}
...
contract Registry is Storage {
    ...
    function setDelegateAddress (address _c) public {
        require(msg.sender == owner, "Only the owner can set the delegate address");
        delegate = _c;
    }
